...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> { 
		...
		pub fn grant_access(
			...
			// Check if the account already have access
			if let Some(existing_who) = ExtrinsicAccess::<T>::get((pallet_index, extrinsic_name.clone())) {
				if existing_who == who {
					return Err(Error::<T>::AlreadyGranted.into());
				}
			}
	...
	#[cfg(test)]
	mod test {
		use crate as aga_access_segregator;
		use crate::{
			mock::{
		...
		fn should_work() {
			new_test_ext().execute_with(|| {
				assert_noop!(
					...
					aga_access_segregator::Error::<Test>::GrantAccessFailed
				);
